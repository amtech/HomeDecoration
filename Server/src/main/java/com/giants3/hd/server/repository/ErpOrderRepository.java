package com.giants3.hd.server.repository;

import com.giants3.hd.utils.DateFormats;
import com.giants3.hd.utils.entity_erp.ErpOrder;
import com.giants3.hd.utils.entity_erp.ErpOrderItem;
import com.giants3.hd.utils.entity_erp.Prdt;
import org.springframework.data.domain.Pageable;
import org.springframework.data.repository.query.Param;

import javax.persistence.EntityManager;
import javax.persistence.Query;
import javax.persistence.TypedQuery;
import javax.persistence.criteria.CriteriaBuilder;
import javax.persistence.criteria.CriteriaQuery;
import javax.persistence.criteria.Root;
import java.sql.Date;
import java.sql.Timestamp;
import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;

/**
 * 从第三方数据库读取erpOrder数据相关 订单
 */
public class ErpOrderRepository {


    public static final String KEY_ORDER = "SO";
    private EntityManager em;

    public static final String SQL_ORDER_LIST = "select a.*,b.so_data from  (select p.os_dd    ,CAST(p.os_no AS varchar) as os_no ,CAST(p.cus_no AS varchar) as cus_no ,CAST(p.cus_os_no AS varchar) as  cus_os_no ,CAST(p.rem AS varchar) as rem ,p.est_dd  from  MF_POS p where p.OS_ID=:OS_ID and p.OS_NO like :OS_NO)  a  LEFT JOIN\n" +
            "  \n" +
            "(select pz.os_id,pz.os_no,pz.so_data from  MF_POS_Z pz where pz.OS_ID=:OS_ID and pz.OS_NO like :OS_NO )   b on a.os_no=b.os_no   order by a.os_no,a.est_dd DESC ";

    public static final String SQL_ORDER_LIST_COUNT = " select  count(p.os_no)  from  MF_POS p where p.OS_ID=:OS_ID and p.OS_NO like :OS_NO  ";

    public static String sql_item_1 = "   select os_no,itm,bat_no,prd_no,prd_name,id_no, up,qty,amt from tf_pos  where os_id=:OS_ID and os_no =:OS_NO ";
    public static String sql_item_2 = " select htxs,so_zxs,khxg,xgtj,zxgtj,hpgg ,os_no,itm from tf_pos_z    where os_id=:OS_ID and os_no =:OS_NO ";
    public static final String SQL_ORDER_ITEM_LIST = "  select  a.* ,b.* from (  " + sql_item_1 + ") a left outer join (" + sql_item_2 + ") b on a.os_no=b.os_no and a.itm=b.itm order by a.os_no ,a.itm asc";

    public ErpOrderRepository(EntityManager em) {
        this.em = em;
    }


    /*
    *   查询订单
    *   返回2个元素数组    【0】 表示查询记录数
    *       【1】 表示查询的结果列表  List<ErpOrder>
     */
    public List<ErpOrder> findOrders(String os_no, int pageIndex, int pageSize) {



        Query query= em.createNativeQuery(SQL_ORDER_LIST).setParameter("OS_ID", KEY_ORDER).setParameter("OS_NO", "%" + os_no + '%');

        List result = query.setFirstResult(pageIndex * pageSize).setMaxResults(pageSize).getResultList();
        return  convertToPojo(result);



    }

    /**
     * 获取关键字查询记录总数
     * @param key
     * @return
     */
    public int getOrderCountByKey(String key)
    {
     return (Integer )em.createNativeQuery(SQL_ORDER_LIST_COUNT).setParameter("OS_ID", KEY_ORDER).setParameter("OS_NO", "%" + key + '%').getSingleResult();
    }

    /**
     * 查看订单item
     * @param orderNo
     * @return
     */
    public List<ErpOrderItem> findItemsByOrderNo(String orderNo) {


        List result = em.createNativeQuery(SQL_ORDER_ITEM_LIST).setParameter("OS_ID", KEY_ORDER).setParameter("OS_NO",  orderNo  ).getResultList();
        return convertToOrderItem(result);


    }


    /**
     * 数据转换  转换成{@Link ErpOrderItem }
     *
     * @param result
     * @return
     */
    private List<ErpOrderItem> convertToOrderItem(List result) {

        List<ErpOrderItem> orderItems = new ArrayList<>();
        Iterator it = result.iterator();

        while (it.hasNext()) {
            Object[] row = (Object[]) it.next(); // Iterating through array object
            // prdts
            ErpOrderItem erpOrderItem = new ErpOrderItem();


            int i = 0;

            //这里解析的顺序跟sql查询语句里 select 先后一致

            erpOrderItem.os_no = row[i] == null ? "" :String.valueOf(row[i]);;
            i++;
            erpOrderItem.itm = row[i] == null ? 0 :Float.valueOf( row[i].toString()).intValue();
            i++;
            erpOrderItem.bat_no = row[i] == null ? "" : row[i].toString();
            i++;
            erpOrderItem.prd_no = row[i] == null ? "" : row[i].toString();
            i++;
            erpOrderItem.prd_name = row[i] == null ? "" : row[i].toString();
            i++;
            erpOrderItem.id_no = row[i] == null ? "" : row[i].toString();
            i++;
            erpOrderItem.up = row[i] == null ? 0 :Float.valueOf( row[i].toString());
            i++;
            erpOrderItem.qty = row[i] == null ? 0 :Float.valueOf( row[i].toString()).intValue();
            i++;
            erpOrderItem.amt = row[i] == null ? 0 :Float.valueOf( row[i].toString());


            i++;
            erpOrderItem.htxs = row[i] == null ? 0 :Integer.valueOf( row[i].toString());

            i++;
            erpOrderItem.so_zxs = row[i] == null ? 0 :Float.valueOf( row[i].toString()).intValue();

            i++;
            erpOrderItem.khxg = row[i] == null ? "" : row[i].toString();

            i++;
            erpOrderItem.xgtj =  row[i] == null ? 0 :Float.valueOf( row[i].toString());
            i++;
            erpOrderItem.zxgtj =  row[i] == null ? 0 :Float.valueOf( row[i].toString());
                 i++;
            erpOrderItem.hpgg =   row[i] == null ? "" : row[i].toString();




            orderItems.add(erpOrderItem);


        }

        return orderItems;

    }

    public List<ErpOrder> convertToPojo(List sqlResult) {

        List<ErpOrder> prdts = new ArrayList<>();
        Iterator it = sqlResult.iterator();

        while (it.hasNext()) {
            Object[] row = (Object[]) it.next(); // Iterating through array object
            // prdts
            ErpOrder order = new ErpOrder();

            // 第一行记录为序号  在分页模式下。
            int i = 0;

            //这里解析的顺序跟sql查询语句里 select 先后一致
            i++;
            order.os_dd = row[i] == null ? "" : DateFormats.FORMAT_YYYY_MM_DD.format((Timestamp) row[i]);
            i++;
            order.os_no = row[i] == null ? "" : row[i].toString();
            i++;
            order.cus_no = row[i] == null ? "" : row[i].toString();
            i++;
            order.cus_os_no = row[i] == null ? "" : row[i].toString();
            i++;
            order.rem = row[i] == null ? "" : row[i].toString();

//            order.price = row[5] == null ? 0 : Float.valueOf(row[5].toString());

            i++;
            order.est_dd = row[i] == null ? "" : DateFormats.FORMAT_YYYY_MM_DD.format((Timestamp) row[i]);
            i++;
            order.so_data = row[i] == null ? "" : DateFormats.FORMAT_YYYY_MM_DD.format((Timestamp) row[i]);

            prdts.add(order);


        }

        return prdts;

    }


}
