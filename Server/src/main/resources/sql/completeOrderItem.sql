...
,e.hpgg,e.khxg, isnull(e.so_zxs,0) as  so_zxs    from
 ...
 (
--9 表示 订单生产中
select osNo,itm,workflowstate,maxWorkFlowStep,maxWorkFlowName, maxWorkFlowCode,workFlowDescribe from  [yunfei].[dbo].[T_OrderItemWorkState] where workflowstate=VALUE_COMPLETE_STATE  and (osNo like :os_no or prdNo like :prd_no)
...
and  os_dd >'2017-01-01' and (os_no like :os_no or prd_no like :prd_no)
 ...
 --生产方式判断
   ...
   (
       --排厂单
      select  distinct  0 as produceType, SO_NO,EST_ITM ,'' as po_no from  MF_MO    where so_no like upper('%YF%') and so_no like :os_no and  ( so_no like :os_no or mrp_no like  :prd_no )
