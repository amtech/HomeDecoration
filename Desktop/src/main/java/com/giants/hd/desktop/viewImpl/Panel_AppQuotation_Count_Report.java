...
import com.giants.hd.desktop.utils.JTableToExcel;
...
public class Panel_AppQuotation_Count_Report extends BasePanel implements AppQuotationCountReportViewer {
 ...
 ;
    ...
    private void createUIComponents() {
        ...
        try {
            long aMonthAgo= Calendar.getInstance().getTimeInMillis()-60l*24*60*60*1000;
            final Calendar instance = Calendar.getInstance();
            instance.setTimeInMillis(aMonthAgo );
            startDate.getJFormattedTextField().setText(formatter.valueToString(instance));
        ...
        try {
            long aMonthAgo= Calendar.getInstance().getTimeInMillis() ;
            final Calendar instance = Calendar.getInstance();
            instance.setTimeInMillis(aMonthAgo );
            endDate.getJFormattedTextField().setText(formatter.valueToString(instance));
        } catch (ParseException e) {
            e.printStackTrace();
        }
    }
