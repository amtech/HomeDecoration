package com.giants.hd.desktop.reports.jasper.product_list_4;
...
public class ProductList4Report extends JRReport {
    ...
    public ProductList4Report(List<Product> products) {
        ...
        for (int i = 0; i < size; i += 2) {
            ...
            data.url = HttpUrl.loadPicture(products.get(index).url);
            data.name = products.get(index).name;
            data.unit = products.get(index).pUnitName;

            int index1 = i + 1;
            if (index1 < size) {
                data.url1 = HttpUrl.loadPicture(products.get(index1).url);
