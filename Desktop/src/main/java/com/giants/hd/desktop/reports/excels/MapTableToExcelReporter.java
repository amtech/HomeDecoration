 ...
 */
public class MapTableToExcelReporter extends SimpleExcelReporter<List<Map >> {
    public static final String FILE_NAME = "MapTable导出模板";
    private static final String TEMPLATE_FILE_NAME = "excel/" + FILE_NAME + ".xls";

    private String fileName;
    private List<TableField> fields;

    public MapTableToExcelReporter( String fileName,List<TableField> fields) {
...
//    }
    ...
    @Override
    protected void report(List<Map> data, Workbook workbook) {
    ...
    @Override
    protected String getDestFileName(List<Map> data) {
        return fileName;
    }
...
//            sheet.addCell(colName);
     ...
     * @param fields
     */
    private void fillCell(Workbook workbook,Sheet sheet, List<Map> datas,List<TableField> fields) {
...
//        format.setAlignment(Alignment.CENTRE); // 水平居中显示
        ...
        for (int i = 0; i < colNum; i++) {    // 列
            ...
            for (int j = 0; j <  rowNum; j++) {// 行
                ...
                Object object = datas.get(j).get(tableField.fileName);
